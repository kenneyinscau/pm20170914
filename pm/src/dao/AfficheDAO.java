 ...
 * Affiche entities. Transaction control of the save(), update() and delete()
...
public class AfficheDAO extends HibernateDaoSupport {
	...
	public Affiche findById(java.lang.Integer id) {
		log.debug("getting Affiche instance with id: " + id);
		try {
			Affiche instance = (Affiche) getHibernateTemplate().get(
					"entity.Affiche", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public Affiche merge(Affiche detachedInstance) {
		...
		try {
			Affiche result = (Affiche) getHibernateTemplate().merge(
					detachedInstance);
	...
	public static AfficheDAO getFromApplicationContext(ApplicationContext ctx) {
		return (AfficheDAO) ctx.getBean("AfficheDAO");
	}
}